...
def make_events_rows():
    return [( 'me','ehr_fhir','Observation','LOINC','8867-4','Heart rate', dt.datetime(2025,9,2,12,0), 105.0, '1/min', {'src':'fhir'} )]
...
def fake_pg_factory(vitals_rows=None, findings_rows=None, events_rows=None):
    ...
    if findings_rows is not None:
        ...
        descs.append([('metric',),('level',),('score',),('context',),('finding_time',)])
    if events_rows is not None:
        seq.append(events_rows)
        descs.append([('person_id',),('source',),('kind',),('code_system',),('code',),('display',),('effective_time',),('value_num',),('unit',),('meta',)])
...
def test_dashboard_render(monkeypatch):
    ...
    monkeypatch.setattr('app.hp_etl.db.pg', lambda *a, **k: fake_pg_factory(vitals, findings)())
...
def test_events_ajax(monkeypatch):
    ...
    monkeypatch.setenv('HP_API_KEY','testkey')
    monkeypatch.setattr('app.hp_etl.db.pg', lambda *a, **k: fake_pg_factory(events_rows=events)())
    # login
