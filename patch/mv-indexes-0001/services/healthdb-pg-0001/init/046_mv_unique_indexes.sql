-- Unique index for analytics.mv_events_daily so it can refresh CONCURRENTLY
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Unique index for analytics.mv_vitals_daily_wide so it can refresh CONCURRENTLY
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes
