...
(function(){
  ...
  function fallbackDraw(cv, data, color, fill){
    ...
    try{
      ...
      ctx.beginPath(); ctx.lineWidth = Math.max(1, Math.round(2*(window.devicePixelRatio||1))); ctx.strokeStyle = color || '#2563eb';
      for(let i=0;i<len;i++){ const x=Math.round((i/(len-1))*cv.width); const v=data[i]==null?null:Number(data[i]); const y=(v==null)?(cv.height/2):Math.round(cv.height-((v-min)/span)*cv.height); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }
      ctx.stroke(); if(fill){ ctx.globalAlpha=0.06; ctx.lineTo(cv.width,cv.height); ctx.lineTo(0,cv.height); ctx.closePath(); ctx.fillStyle=color||'#2563eb'; ctx.fill(); ctx.globalAlpha=1; }
    }catch(e){}
    ctx.restore();
  }
