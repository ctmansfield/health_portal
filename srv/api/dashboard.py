...
async def dashboard(request: Request, person_id: str = 'me', days: int = 14, auth=Depends(require_api_key)):
    since = (dt.datetime.utcnow() - dt.timedelta(days=days)).date().isoformat()
    # cache key
    cache_key = f"dashboard:{person_id}:{days}"
    cached = cache_get(cache_key)
    if cached is not None:
        return templates.TemplateResponse('dashboard.html', {**cached, 'request': request})

    with db.pg() as conn:
        cur = conn.cursor()
        try:
            cur.execute("SELECT day, hr_median, spo2_min FROM analytics.mv_daily_vitals WHERE person_id = %s AND day >= %s::date ORDER BY day ASC", (person_id, since))
            vitals = cur.fetchall()
        except Exception:
            vitals = []
        try:
            cur.execute("SELECT metric, level, score, context, finding_time FROM analytics.ai_findings WHERE person_id = %s AND finding_time >= %s ORDER BY finding_time DESC LIMIT 50", (person_id, since))
    ...
    findings_list = [dict(metric=r[0], level=r[1], score=r[2], context=r[3], finding_time=r[4]) for r in findings]
