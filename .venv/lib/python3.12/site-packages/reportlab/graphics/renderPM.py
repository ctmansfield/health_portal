...
class PMCanvas:
    def __init__(
        ...
        self.__dict__["_colorConverter"] = (
            CairoColor if self._backend == "rlPyCairo" else Color2Hex
        )
    ...
    def toPIL(self):
        im = _getImage().new(
            (
                "RGBA"
                if self._backend == "rlPyCairo" and getattr(self, "_fmt") == "ARGB32"
                else "RGB"
            ),
    ...
    def _saveToBMP(self, f):
        ...
        gs = self._gs
        if (
            self._backend == "rlPyCairo" and gs._fmt == "ARGB32"
        ):  # pixBuf would have 4 bytes
    ...
    def drawCentredString(
        self, x, y, text, text_anchor="middle", direction=None, shaping=False
    ):
        self.drawString(
            x, y, text, text_anchor=text_anchor, direction=direction, shaping=shaping
        )
