...
try:
    ...
    def bidiText(text, direction):
        ...
        return (
            log2vis(text, direction, clean=True)
    ...
    def bidiWordList(words, direction="RTL", clean=True, wx=False):
        ...
        if not isinstance(words, (list, tuple)):
            raise ValueError(
                "bidiWordList argument words should be a list or tuple of strings"
            )
        raw = " ".join(words)
        V2L = []
    ...
    def bidiShapedText(
        ...
        if direction in ("RTL", "LTR"):
            if shaping:
                ...
                bR = len(w) - len(text)
                LW = [
                    text[slice(*m.span())] for m in wordpat.finditer(text)
                ]  # logical words
    ...
    def bidiFragWord(w, direction=None, bidiV=-1, bidiL=-1, clean=True):
        ...
        V2L = []
        bidi = log2vis(
            text, base_direction=direction, clean=clean, positions_V_to_L=V2L
        )
        fsL = [
            (w[1 + k][0], "".join((ks[1] for ks in g)))
            for k, g in groupby(
                ((fL[V2L[i]], v) for i, v in enumerate(bidi)), key=itemgetter(0)
            )
        ]
        bfw = [
            sum((pdfmetrics_stringWidth(s, f.fontName, f.fontSize) for f, s in fsL))
        ] + fsL
        cbd = cbd.__self__
        if cbd:
            if V2L[0] > V2L[-1]:
    ...
    def bidiShapedText(
        ...
        return bidiText(text, direction), pdfmetrics_stringWidth(
            text, fontName, fontSize
        )

    bidiWordList = bidiStrWrap = bidiListWrap = bidiFragWord = innerBidiStrWrap = (
        BidiIndex
    ) = bidiText
    rtlSupport = False
    ...
    def _formatText(self, text):
        ...
        try:
            ...
            if font._dynamicFont:
                ...
                if font.shapable and isinstance(text, ShapedStr):
                    ...
                    for subset, t in font.splitString(text, canv._doc):
                        ...
                        if subset != self._curSubset:
                            if not tmpl:
                                tmpl = (
                                    f"{fp_str(fontsize)} Tf {fp_str(self._leading)} TL"
                                )
                ...
                canv_escape = canv._escape
                for f, t in pdfmetrics_unicode2T1(
                    text, [font] + font.substitutionFonts
                ):
                    if f != fc:
                        R(
                            "%s %s Tf %s TL"
