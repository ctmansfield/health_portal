...
OUT_BASE = os.environ.get("PORTAL_OUTDIR_BASE", "/mnt/nas_storage/ingest/portal_ingest_out")
...
def main():
    ...
    run_id = uuid.uuid4()
    outdir = os.path.join(OUT_BASE, str(run_id))
    os.makedirs(outdir, exist_ok=True)

    all_rows=[]; rejects=[]; dups=[]; seen=set()
    for p in args.input:
        if not os.path.exists(p):
            print(f"[warn] missing input {p}", flush=True); continue
        ...
        for r in rows:
            ...
            if v is None:
                rejects.append(dict(reason="non-numeric or SEE NOTE", provider=r.get('provider'),
                                    raw_text=r.get('src_line'), effective_time=r.get('effective_time'),
    ...
    # staged.csv
    tmp = os.path.join(outdir,".staged.tmp"); final = os.path.join(outdir,"staged.csv")
    ...
    # duplicates.csv
    tmp = os.path.join(outdir,".duplicates.tmp"); final = os.path.join(outdir,"duplicates.csv")
    with open(tmp, "w", newline='') as f:
        w = csv.DictWriter(f, fieldnames=["run_id","src_hash","reason","details"], quoting=csv.QUOTE_ALL, lineterminator='\n')
        w.writeheader()
        for d in dups: w.writerow({"run_id":str(run_id), **d})
    os.replace(tmp, final)

    # rejections.csv
    tmp = os.path.join(outdir,".rejections.tmp"); final = os.path.join(outdir,"rejections.csv")
    with open(tmp, "w", newline='') as f:
        w = csv.DictWriter(f, fieldnames=["run_id","reason","provider","raw_text","effective_time","parsed"], quoting=csv.QUOTE_ALL, lineterminator='\n')
        w.writeheader()
