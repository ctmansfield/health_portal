...
OUT_BASE = os.environ.get(
    "PORTAL_OUTDIR_BASE", "/mnt/nas_storage/ingest/portal_ingest_out"
)
MONTHS = {
    "jan": 1,
    "feb": 2,
...
def main():
    ...
    # staged.csv
    tmp = os.path.join(outdir, ".staged.tmp")
    final = os.path.join(outdir, "staged.csv")
    ...
    # duplicates.csv
    tmp = os.path.join(outdir, ".duplicates.tmp")
    final = os.path.join(outdir, "duplicates.csv")
    with open(tmp, "w", newline="") as f:
        w = csv.DictWriter(
            f,
            fieldnames=["run_id", "src_hash", "reason", "details"],
    ...
    # rejections.csv
    tmp = os.path.join(outdir, ".rejections.tmp")
    final = os.path.join(outdir, "rejections.csv")
    with open(tmp, "w", newline="") as f:
        w = csv.DictWriter(
            f,
            fieldnames=[
